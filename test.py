...
if __name__ == "__main__":
    ...
    profobj = cProfile.Profile()
    profobj.enable()
    calc_pure_python(draw_output=False, desired_width=100, max_iterations=10)
    profobj.disable()
    a = profobj.getstats()
    print type(a[0].code)
    print a[0].code
    cumtime = []
    [cumtime.append(x.totaltime) for x in a]
    for x in cumtime:
        print '{:.3f}'.format(x)
    #print cumtime
    import pstats
    pstats.Stats(profobj).sort_stats('cumulative').print_stats()
    ign_str = '''
    a = pstats.Stats(profobj).sort_stats('tottime')
    print type(a.__class__)
    exit
    for item in a:
        print '{:.3f}'.formar(item)
    '''
    #print a.sort_stats('tottime')
    #a = profobj.getstats()
    #a = profobj.print_stats()
    #print profobj.sort_stats('tottime')
    #profobj.dump_stats('output.pstats')
